...
interface ShippingRequest {
  ...
  preferredCurrency?: string; // FIXED: Added optional preferredCurrency parameter
  fedexConfig?: {
...
// FIXED: Updated FedEx API payload to use user-selected preferred currency
async function getFedexRates(
  ...
  userPreferredCurrency?: string // FIXED: Added preferred currency parameter
  ...
  return retryWithBackoff(async () => {
    ...
    // FIXED: Use user-selected currency first, then fall back to auto-mapping
    ...
    Logger.info('Currency selection for FedEx request', {
      userSelected: userPreferredCurrency,
      fallbackMapped: userPreferredCurrency ? null : preferredCurrency,
    ...
    const payload = {
      ...
      requestedShipment: {
        ...
        preferredCurrency: preferredCurrency // FIXED: Use user-selected or mapped currency
      }
    };

    Logger.info('Sending FedEx rate request with user-selected currency', { 
      payload: {
        ...payload,
    ...
    try {
      ...
      Logger.info(`Successfully parsed ${rates.length} FedEx rates in ${preferredCurrency}`);
      return rates;
    ...
    if (requestData.fedexConfig) {
      ...
      try {
        ...
        const fedexRates = await getFedexRates(
          ...
          requestData.preferredCurrency // FIXED: Pass user-selected currency
        );
