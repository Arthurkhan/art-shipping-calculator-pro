      ...
      try {
        for (const rateDetail of responseData.output.rateReplyDetails) {
          if (rateDetail.ratedShipmentDetails && rateDetail.ratedShipmentDetails.length > 0) {
            // FIX: The totalNetCharge is directly on the ratedShipmentDetail object
            ...
            if (shipmentDetail.totalNetCharge) {
              rateAmount = parseFloat(shipmentDetail.totalNetCharge) || 0;
              rateCurrency = shipmentDetail.currency || currency;
              Logger.info('Found rate at correct location', { 
                rateAmount, 
                rateCurrency,
                service: rateDetail.serviceType
              });
            }
            ...
            if (rateAmount > 0) {
              const rate: ShippingRate = {
                ...
                transitTime: rateDetail.transitTime || 
                             rateDetail.operationalDetail?.transitTime || 
                             'Unknown',
                deliveryDate: rateDetail.deliveryTimestamp || 
                             rateDetail.operationalDetail?.deliveryDate
              };
