...
export const SizeSelector = ({
  ...
  return (
    <div className="space-y-1.5">
      <Label htmlFor="size" className="text-sm text-slate-700 dark:text-slate-300 font-medium flex items-center">
        <Ruler className="w-3.5 h-3.5 mr-1.5" />
        Artwork Size
      </Label>
      <Select value={selectedSize} onValueChange={onSizeChange} disabled={disabled}>
        <SelectTrigger id="size" className="w-full h-10 border-slate-300 focus:border-blue-500 focus:ring-blue-500 disabled:bg-slate-100 dark:disabled:bg-slate-800">
